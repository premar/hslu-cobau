...
public class HTMLReport implements Report {
    ...
    public void footer() {
        ...
        double succeeded_score = 0;

            for(TestResult testResult : results) {
                if (testResult.getStatus() == TestStatus.PASSED) {
                    succeeded_score += testResult.getTestCase().getScore();
                }
                total_score += testResult.getTestCase().getScore();
            }

            double score;
            if (total_score != 0) {
                score = succeeded_score;
            } else {
                score = 1.0;
            }
            ...
            sb.append("</head>");
            sb.append("<body>");
            sb.append("<p><b>Test results (score: ").append(score).append(" of ").append(total_score).append("):</b></p>");
            sb.append("<table>");
            sb.append("  <col width=\"14%\">");
            sb.append("  <col width=\"14%\">");
            sb.append("  <col width=\"24%\">");
            sb.append("  <col width=\"23%\">");
